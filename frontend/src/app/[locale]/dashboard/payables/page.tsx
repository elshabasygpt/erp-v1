...
export default function PayablesPage() {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const res = await api.get('/crm/payables/aging');
            if (res.data?.success) {
            ...
            <div className="flex justify-between items-center bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800 dark:text-white">
                        {isRTL ? 'إدارة الذمم الدائنة (أعمار الديون)' : 'Payables Aging Report'}
                    </h1>
                    <p className="text-slate-500 text-sm mt-1">
                        {isRTL ? 'متابعة الديون المستحقة للموردين وتصنيفها حسب فترة التأخير' : 'Track outstanding supplier balances and aging categories'}
                    </p>
                </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700">
                            <tr>
                                <th className="px-6 py-4 text-sm font-bold text-slate-700 dark:text-slate-300">{isRTL ? 'المورد' : 'Supplier'}</th>
                            ...
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={6} className="px-6 py-8 text-center text-slate-400 italic">
                                        {isRTL ? 'لا توجد ديون مستحقة للموردين' : 'No outstanding payables found'}
                                    </td>
                                </tr>
                            ) : (
                                data.map((item, idx) => (
