...
export default function VehicleManagementPage({ params }: { params: { locale: string } }) {
    ...
    const selectMake = async (make: any) => {
        ...
        setLoading(true);
        try {
            const res = await inventoryApi.getVehicleModels(make.make_id);
            setModels(res.data);
        } catch (e) {
            console.error(e);
        } finally {
    ...
    const selectModel = async (model: any) => {
        ...
        setLoading(true);
        try {
            const res = await inventoryApi.getVehicleYears(model.model_id);
            setYears(res.data);
        } catch (e) {
            console.error(e);
        } finally {
    ...
    const handleCreateModel = async () => {
        ...
        try {
            await inventoryApi.createVehicleModel(activeMake.make_id, newModel);
    ...
    const handleCreateYear = async () => {
        ...
        try {
            await inventoryApi.createVehicleYear(activeModel.model_id, newYear);
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-2">
                        {loading && !activeMake ? <Loader2 className="w-6 h-6 animate-spin mx-auto text-blue-500 my-4" /> : makes.map(make => (
                            <button
                                ...
                                className={`w-full text-start px-4 py-3 rounded-xl border font-bold transition-all flex justify-between items-center ${activeMake?.make_id === make.make_id ? 'bg-blue-50 border-blue-200 text-blue-700 dark:bg-blue-600/20 dark:border-blue-500/30 dark:text-blue-400' : 'bg-white border-slate-100 text-slate-600 hover:border-blue-300 dark:bg-[#151522] dark:border-white/5 dark:text-white/80 dark:hover:border-blue-500/50'}`}
                            >
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-2">
                        {loading && activeMake && !activeModel ? <Loader2 className="w-6 h-6 animate-spin mx-auto text-blue-500 my-4" /> : models.map(model => (
                            <button
                                ...
                                className={`w-full text-start px-4 py-3 rounded-xl border font-bold transition-all flex justify-between items-center ${activeModel?.model_id === model.model_id ? 'bg-amber-50 border-amber-200 text-amber-700 dark:bg-amber-600/20 dark:border-amber-500/30 dark:text-amber-400' : 'bg-white border-slate-100 text-slate-600 hover:border-amber-300 dark:bg-[#151522] dark:border-white/5 dark:text-white/80 dark:hover:border-amber-500/50'}`}
                            >
                ...
                <div className={`bg-white dark:bg-[#111118] border border-slate-200 dark:border-white/10 rounded-2xl flex flex-col overflow-hidden shadow-sm transition-opacity ${!activeModel ? 'opacity-50 pointer-events-none' : ''}`}>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-2">
                        {loading && activeModel ? <Loader2 className="w-6 h-6 animate-spin mx-auto text-blue-500 my-4" /> : years.map(year => (
                            <div key={year.vehicle_year_id} className="w-full px-4 py-3 rounded-xl border bg-white border-slate-100 text-slate-600 dark:bg-[#151522] dark:border-white/5 dark:text-white/80 font-bold flex justify-between items-center">
                                <div>
