        ...
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="glass-card rounded-2xl overflow-hidden">
                <div className="px-6 py-5 border-b border-surface-200 dark:border-surface-700 flex flex-wrap justify-between items-center gap-3">
                    <div>
                        ...
                        <p className="text-sm text-surface-500 mt-0.5">{isRTL ? 'الأصناف التي ستنفد قريباً بناءً على معدل المبيعات' : 'Items predicted to run out based on sales velocity'}</p>
                    </div>
                    <div className="flex gap-2 items-center">
                        <div>
                            <label className="text-xs text-surface-500 block mb-1">{isRTL ? 'حد التنبيه' : 'Alert Threshold'}</label>
                            <input type="number" min="1" max="100" value={threshold} onChange={e => setThreshold(parseInt(e.target.value))}
                                className="w-20 p-2 border rounded-lg bg-surface-50 dark:bg-surface-900 border-surface-200 dark:border-surface-700 text-sm"/>
                        </div>
                    ...
                    <div className="p-6">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-surface-500">{isRTL ? 'بناءً على معدل المبيعات الحالي' : 'Based on current sales velocity'}</p>
                            </div>
                        </div>
                                ...
                                {forecastData.forecasts.map((f: any, i: number) => (
                                    <div key={i} className="flex items-center justify-between p-4 bg-red-50 dark:bg-red-900/20 rounded-xl border border-red-200 dark:border-red-800">
                                        <div>
                                            ...
                                            <p className="text-xs text-surface-500 mt-0.5">{isRTL ? 'المخزون الحالي:' : 'Stock:'} <strong>{f.current_stock || f.quantity}</strong> | {isRTL ? 'متوسط المبيعات:' : 'Avg. Sales:'} <strong>{parseFloat(f.avg_daily_sales || 0).toFixed(1)}/day</strong></p>
                                        </div>
                                        <div className="text-right">
                                            ...
                                            <p className="text-xs text-surface-500">{isRTL ? 'حتى النفاد' : 'until empty'}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
            ...
            <div className="glass-card rounded-2xl overflow-hidden">
                <div className="px-6 py-5 border-b border-surface-200 dark:border-surface-700">
                    ...
                    <p className="text-sm text-surface-500 mt-0.5">{isRTL ? 'يقوم النظام بإنشاء مسودة أمر شراء تلقائياً للأصناف الناقصة' : 'System auto-generates a purchase order draft for depleted items'}</p>
                </div>
                <div className="p-6">
                    <div className="flex gap-3 items-end mb-5">
                        <div className="flex-1">
                            ...
                            <select value={selectedWarehouse} onChange={e => setSelectedWarehouse(e.target.value)}
                                className="w-full p-2.5 border rounded-lg bg-surface-50 dark:bg-surface-900 border-surface-200 dark:border-surface-700 text-sm">
                        ...
                        <button onClick={runAutoPO} disabled={loading.autoPO || !selectedWarehouse}
                            ...
                            {loading.autoPO ? '...' : `🤖 ${isRTL ? 'إنشاء تلقائي' : 'Auto Generate'}`}
                        </button>
                    </div>
            ...
            {/* Partner Forecast */}
            <div className="glass-card rounded-2xl overflow-hidden">
                <div className="px-6 py-5 border-b border-surface-200 dark:border-surface-700 flex justify-between items-center">
                    <div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                                {[
                                    ...
                                    <div key={m.label_en} className="p-4 bg-purple-50 dark:bg-purple-900/20 rounded-xl border border-purple-200 dark:border-purple-800">
                                        <p className="text-xs text-surface-500">{isRTL ? m.label_ar : m.label_en}</p>
                        ...
                        {partnerForecast.partner_projections?.length > 0 ? (
                            <div className="space-y-3">
                                {partnerForecast.partner_projections.map((p: any) => (
                                    <div key={p.partner_id} className="flex items-center justify-between p-4 bg-surface-50 dark:bg-surface-900 rounded-xl border border-surface-200 dark:border-surface-700">
                                        <div>
                                            <p className="font-medium">{p.partner_name || 'Partner'}</p>
                                            <p className="text-xs text-surface-500 mt-0.5">{p.profit_share_pct}% {isRTL ? 'حصة' : 'share'}</p>
                                        </div>
                                        <div className="text-right">
                            ...
                            <p className="text-center text-surface-400 py-4">{isRTL ? 'لا توجد بيانات شركاء' : 'No partner data available'}</p>
                        )}
                    </div>
                )}
            </div>
