    ...
    const { data: products = [], isLoading: isLoadingProducts, error } = useQuery({
        ...
        queryFn: async () => {
            ...
            return res.data.data.map((p: any) => ({
                ...
                wholesalePrice: parseFloat(p.wholesale_price || '') || parseFloat(p.sell_price || 0) * 0.9,
                semiWholesalePrice: parseFloat(p.semi_wholesale_price || '') || parseFloat(p.sell_price || 0) * 0.95,
                profitPercent: 0,
                discount: 0,
