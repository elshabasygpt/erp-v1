            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 flex-1 min-h-[600px]">
                ...
                <div className="bg-white dark:bg-[#111118] border border-slate-200 dark:border-white/10 rounded-2xl flex flex-col overflow-hidden shadow-sm">
                        ...
                        {loading && !activeMake ? <Loader2 className="w-6 h-6 animate-spin mx-auto text-blue-500 my-4" /> : makes.map(make => (
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    {make.logo_url && <img src={process.env.NEXT_PUBLIC_API_URL?.replace('/api', '') + make.logo_url} alt="" className="w-8 h-8 object-contain rounded" />}
                                    <span>{isRTL ? make.name_ar : make.name}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <button onClick={(e) => { e.stopPropagation(); setEditingMakeId(make.id); setNewMake({ name: make.name, name_ar: make.name_ar }); setMakeLogo(null); }} className="p-1.5 text-slate-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-500/20 rounded-lg transition-colors">
                    ...
                    <div className="p-4 border-t border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/5 space-y-2">
                        ...
                        <label className="flex items-center gap-2 w-full h-10 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-[#1a1a2e] text-sm cursor-pointer text-slate-500">
                            <input type="file" accept="image/*" className="hidden" onChange={e => setMakeLogo(e.target.files?.[0] || null)} />
                            <span className="truncate">{makeLogo ? makeLogo.name : (isRTL ? 'رفع الشعار (اختياري)' : 'Upload Logo (Optional)')}</span>
                        </label>
                        <div className="flex gap-2">
                            ...
                            {editingMakeId && (
                                <button onClick={() => { setEditingMakeId(null); setNewMake({ name: '', name_ar: '' }); setMakeLogo(null); }} className="px-4 h-10 bg-slate-200 text-slate-700 font-bold rounded-lg flex items-center justify-center">
                                    <X className="w-4 h-4" />
                                </button>
                            )}
                        </div>
                ...
                <div className={`bg-white dark:bg-[#111118] border border-slate-200 dark:border-white/10 rounded-2xl flex flex-col overflow-hidden shadow-sm transition-opacity ${!activeMake ? 'opacity-50 pointer-events-none' : ''}`}>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-2">
                        {loading && activeMake && !activeModel ? <Loader2 className="w-6 h-6 animate-spin mx-auto text-blue-500 my-4" /> : models.map(model => (
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    {model.image_url && <img src={process.env.NEXT_PUBLIC_API_URL?.replace('/api', '') + model.image_url} alt="" className="w-8 h-8 object-cover rounded" />}
                                    <span>{isRTL ? model.name_ar : model.name}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <button onClick={(e) => { e.stopPropagation(); setEditingModelId(model.id); setNewModel({ name: model.name, name_ar: model.name_ar }); setModelImage(null); }} className="p-1.5 text-slate-400 hover:text-amber-500 hover:bg-amber-50 dark:hover:bg-amber-500/20 rounded-lg transition-colors">
                                        <Edit2 className="w-4 h-4" />
                                    </button>
                    ...
                    <div className="p-4 border-t border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/5 space-y-2">
                        ...
                        <label className="flex items-center gap-2 w-full h-10 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-[#1a1a2e] text-sm cursor-pointer text-slate-500">
                            <input type="file" accept="image/*" className="hidden" onChange={e => setModelImage(e.target.files?.[0] || null)} />
                            <span className="truncate">{modelImage ? modelImage.name : (isRTL ? 'صورة الموديل (اختياري)' : 'Model Image (Optional)')}</span>
                        </label>
                        <div className="flex gap-2">
                            ...
                            {editingModelId && (
                                <button onClick={() => { setEditingModelId(null); setNewModel({ name: '', name_ar: '' }); setModelImage(null); }} className="px-4 h-10 bg-slate-200 text-slate-700 font-bold rounded-lg flex items-center justify-center">
                                    <X className="w-4 h-4" />
                                </button>
                            )}
                        </div>
                ...
                <div className={`bg-white dark:bg-[#111118] border border-slate-200 dark:border-white/10 rounded-2xl flex flex-col overflow-hidden shadow-sm transition-opacity ${!activeModel ? 'opacity-50 pointer-events-none' : ''}`}>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-2">
                        {loading && activeModel ? <Loader2 className="w-6 h-6 animate-spin mx-auto text-blue-500 my-4" /> : years.map(year => (
                            <div key={year.id} className="w-full px-4 py-3 rounded-xl border bg-white border-slate-100 text-slate-600 dark:bg-[#151522] dark:border-white/5 dark:text-white/80 font-bold flex justify-between items-center">
                                <div className="flex items-center gap-3">
                                    {year.engine_image_url && <img src={process.env.NEXT_PUBLIC_API_URL?.replace('/api', '') + year.engine_image_url} alt="" className="w-8 h-8 object-cover rounded" />}
                                    <div>
                                ...
                                <div className="flex items-center gap-2">
                                    <button onClick={(e) => { e.stopPropagation(); setEditingYearId(year.id); setNewYear({ year_from: year.year_from?.toString() || '', year_to: year.year_to?.toString() || '', engine_size: year.engine_size || '' }); setEngineImage(null); }} className="p-1.5 text-slate-400 hover:text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-500/20 rounded-lg transition-colors">
                                        <Edit2 className="w-4 h-4" />
                                    </button>
                    ...
                    <div className="p-4 border-t border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/5 space-y-2">
                        ...
                        <label className="flex items-center gap-2 w-full h-10 px-3 rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-[#1a1a2e] text-sm cursor-pointer text-slate-500">
                            <input type="file" accept="image/*" className="hidden" onChange={e => setEngineImage(e.target.files?.[0] || null)} />
                            <span className="truncate">{engineImage ? engineImage.name : (isRTL ? 'صورة المحرك (اختياري)' : 'Engine Image (Optional)')}</span>
                        </label>
                        <div className="flex gap-2">
                            ...
                            {editingYearId && (
                                <button onClick={() => { setEditingYearId(null); setNewYear({ year_from: '', year_to: '', engine_size: '' }); setEngineImage(null); }} className="px-4 h-10 bg-slate-200 text-slate-700 font-bold rounded-lg flex items-center justify-center">
                                    <X className="w-4 h-4" />
                                </button>
                            )}
                        </div>
