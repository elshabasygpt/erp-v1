    ...
    return (
        <div className="overflow-x-auto">
            <table className="data-table text-sm">
                <thead><tr><th>{inv.itemCode}</th><th>{inv.itemName}</th><th>{inv.mainGroup}</th><th>{inv.unit}</th><th>{inv.costPrice}</th><th>{inv.sellPrice}</th><th>{inv.wholesalePrice}</th><th>{inv.stock}</th><th>{inv.minStock}</th><th>{common.actions}</th></tr></thead>
                <tbody>
                    {filtered.map(p => {
