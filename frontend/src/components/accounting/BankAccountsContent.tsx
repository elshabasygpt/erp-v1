...
export default function BankAccountsContent({ dict, locale }: { dict: any; locale: string }) {
    ...
    const { data: reconciliations = [] } = useQuery<any[]>({
        queryKey: ['bank-accounts', 'reconciliations', selectedBank?.id],
        queryFn: async () => {
    ...
    const loadBanks = () => queryClient.invalidateQueries({ queryKey: ['bank-accounts'] });
    ...
    const handleCreateOrUpdateBank = async (e: React.FormEvent) => {
        ...
        try {
            if (editingBank) {
                ...
                toast.success(isRTL ? 'تمت الإضافة بنجاح' : 'Created successfully');
            }
            setIsFormOpen(false);
            setEditingBank(null);
    ...
    const handleDeleteBank = async (bank: any) => {
        ...
        try {
            ...
            toast.error(error?.response?.data?.message || 'Cannot delete bank account');
        }
    };
    ...
    const handleViewReconciliations = (bank: any) => {
                        ...
                        <div key={bank.id} className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-700 hover:border-blue-300 dark:hover:border-blue-700 transition">
                            <div className="flex items-start justify-between mb-4">
                                ...
                                <div className="flex gap-2">
                                    <button onClick={() => handleEditBankClick(bank)} className="text-slate-400 hover:text-blue-500 transition" title={isRTL ? 'تعديل' : 'Edit'}>
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" /></svg>
                                    </button>
                                    <button onClick={() => handleDeleteBank(bank)} className="text-slate-400 hover:text-red-500 transition" title={isRTL ? 'حذف' : 'Delete'}>
