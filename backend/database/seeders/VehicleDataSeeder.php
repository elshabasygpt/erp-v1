...
class VehicleDataSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $tenantId = DB::connection('pgsql')->table('tenants')->value('id');
        ...
        DB::connection('tenant')->table('product_vehicle_compatibility')->delete();
        DB::connection('tenant')->table('vehicle_years')->delete();
        DB::connection('tenant')->table('vehicle_models')->delete();
        DB::connection('tenant')->table('vehicle_makes')->delete();
        ...
        DB::connection('tenant')->transaction(function () use ($makes, $tenantId) {
            foreach ($makes as $makeName => $makeData) {
                $makeId = Str::uuid()->toString();
                DB::connection('tenant')->table('vehicle_makes')->insert([
                    'id' => $makeId,
                    'tenant_id' => $tenantId,
                    'name' => $makeName,
                ...
                foreach ($makeData['models'] as $modelName => $modelData) {
                    ...
                    DB::connection('tenant')->table('vehicle_models')->insert([
                        'id' => $modelId,
                        'tenant_id' => $tenantId,
                        'make_id' => $makeId,
                        'name' => $modelName,
                    ...
                    foreach ($modelData['years'] as $yearRange) {
                        ...
                        DB::connection('tenant')->table('vehicle_years')->insert([
