...
class ExchangeRateService
{
    ...
    {
        $base = CurrencyModel::query()->where('tenant_id', $tenantId)->where('is_base', true)->first();
        if (! $base) {
    ...
    {
        $currency = CurrencyModel::query()->where('tenant_id', $tenantId)->find($currencyId);
        if (! $currency) {
        ...
        $rateRecord = ExchangeRateModel::query()->where('tenant_id', $tenantId)
