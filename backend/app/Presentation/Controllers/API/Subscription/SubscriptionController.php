...
class SubscriptionController extends BaseTenantController
{
    ...
    public function current(Request $request)
    {
        ...
        $tenant = DB::connection(env('DB_CONNECTION', 'pgsql'))->table('tenants')->where('id', $tenantId)->first();
        if (! $tenant) {
            return $this->error('Tenant not found', 404);
        }

        $subscription = DB::connection(env('DB_CONNECTION', 'pgsql'))->table('subscriptions')
            ->where('tenant_id', $tenantId)
            ->where('status', 'active')
            ->first();

        $plans = DB::connection(env('DB_CONNECTION', 'pgsql'))->table('plans')->where('is_active', true)->get();
    ...
    public function checkout(Request $request)
    {
        $validated = $request->validate([
            'plan_id' => 'required|exists:'.env('DB_CONNECTION', 'pgsql').'.plans,id',
        ...
        DB::connection(env('DB_CONNECTION', 'pgsql'))->table('subscriptions')->updateOrInsert(
            ['tenant_id' => $tenantId],
            [
