...
class WebhookController extends BaseTenantController
{
    ...
    public function store(Request $request)
    {
        ...
        $endpoint = WebhookEndpointModel::query()->create($validated);
    ...
    {
        $endpoint = WebhookEndpointModel::query()->where('tenant_id', $this->getTenantId($request))->findOrFail($id);
    ...
    {
        $logs = WebhookLogModel::query()->where('tenant_id', $this->getTenantId($request))->where('endpoint_id', $id)
