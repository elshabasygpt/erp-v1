...
class PartnerForecastingService
{
     ...
     */
    public function getEndOfYearProjections(string $tenantId): array
    {
        ...
        $totalSystemProfit = DB::connection('tenant')
            ->table('invoice_items')
        ...
        $partners = PartnerModel::query()->where('tenant_id', $tenantId)->where('is_active', true)->get();
        $projections = [];
